...
from models.graph_learning_module import GraphLearningModule
...
class SpeqGNN(nn.Module):
	...
	def __init__(self, n_channels=1, n_classes=1, *,
		...
		self.glm = GraphLearningModule(input_dim=512, k=glm_k)
	...
	def forward(self, x_magnitude, complex_spec_input, original_length=None):
		...
		# GNNによるノード特徴の更新
		x4_processed_flat = self.gnn(x4_reshaped, edge_index, edge_weight=edge_weight)
...
def print_model_summary(model, batch_size, channels, length):
	...
	x_magnitude_spec = torch.stft(x_time.squeeze(1), n_fft=n_fft, hop_length=hop_length, win_length=win_length, window=window,
	                              return_complex=False)
	x_magnitude_spec = torch.sqrt(x_magnitude_spec[..., 0] ** 2 + x_magnitude_spec[..., 1] ** 2).unsqueeze(1)  # (B, 1, F, T_spec)

	# 複素スペクトログラム (B, F, T_spec)
	x_complex_spec = torch.stft(x_time.squeeze(1), n_fft=n_fft, hop_length=hop_length, win_length=win_length, window=window,
	                            return_complex=True)
...
def main():
	...
	for gnn_type in gnn_types:
		...
		model_type = f"{gnn_type}_{num_node_edges}edges"

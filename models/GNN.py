...
class UGNN(nn.Module):
	...
	def forward(self, x, export_name="graph_data.npz", out_dir=None):
		# エンコーダ
		...
		# GNN処理
		# x4_processed = self.gnn(x4_nodes, edge_index) # 解析前
		""" ↓↓↓ 解析後 """
		# GNN処理（アテンション取得対応）
		if self.gnn_type == "GAT" :
			x4_processed, att_list = self.gnn(x4_nodes, edge_index)
		else:
		...
		# if export_graph:
		# 時間軸の座標 (0, 1, 2, ... L')
		coords = torch.arange(length).unsqueeze(1).repeat(batch_size, 1)
		self._save_graph_data(export_name, edge_index, x4_nodes, coords, att_list, out_dir=out_dir)
		""" ↑↑↑ 解析後 """

...
class GAT(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        x = self.conv3(
            x, edge_index
        )  # 最終層の出力はそのまま（softmaxなどはモデル全体の後段で）
        return x
...
class UGCN(nn.Module):
    ...
    def forward(self, x, edge_index=None):
        ...
        x4_reshaped = (
            x4.view(batch_size, channels_bottleneck, -1)
            .permute(0, 2, 1)
            .reshape(-1, channels_bottleneck)
        )
        ...
        x4_processed = x4_processed_flat.view(
            batch_size, height_bottleneck, width_bottleneck, channels_bottleneck
        ).permute(0, 3, 1, 2)
        ...
        mask_pred = torch.sigmoid(
            self.outc(d1)
        )  # [B, n_classes, H_mask, W_mask] (通常 H_mask=encoder_dim, W_mask=L_encoded)

        # マスクをx_encodedの次元に合わせる (n_classes=1を想定)
        if mask_pred.size(2) != x_encoded.size(1) or mask_pred.size(
            3
        ) != x_encoded.size(2):
            mask_pred_resized = F.interpolate(
                mask_pred,
                size=(x_encoded.size(1), x_encoded.size(2)),
                mode="bilinear",
                align_corners=False,
            )
        else:
        ...
        if self.n_classes == 1:
            ...
            # ここでは単純な要素積を仮定
            masked_x_encoded = (
                x_encoded * mask_pred_resized
            )  # 要確認: n_classesとencoder_dimの関係
...
class UGAT(UGCN):  # UGCNNetを継承
    def __init__(
        ...
        super(UGAT, self).__init__(n_channels, n_classes, hidden_dim, num_node)
...
class UGCN2(UGCN):
    ...
    def forward(self, x):  # edge_indexは内部で生成するため引数から削除
        ...
        if num_nodes_per_sample > 0:
            edge_index = self.create_graph(
                x4_reshaped, self.num_node, batch_size, num_nodes_per_sample
            )
        else:
            edge_index = torch.empty(
                (2, 0), dtype=torch.long, device=x4_reshaped.device
            )
        ...
        # --- ここからデバッグコード ---
        # print(f"--- Debug mask_pred in UGCN2 ---")
...
class UGAT2(UGCN2):  # UGCN2 を継承
    def __init__(
        ...
        # UGCN2 (親クラス) の __init__ を呼び出す
        # UGCN2 は UGCN を継承しており、UGCN で self.gnn が GCN で初期化される
        super(UGAT2, self).__init__(n_channels, n_classes, hidden_dim, num_node)
...
def main():
    ...
    print("\n--- UGCN (Random Graph) ---")
    ugcn_model = UGCN(n_channels=num_mic, n_classes=1, num_node=8).to(device)
    ...
    ugat_model = UGAT(
        n_channels=num_mic, n_classes=1, num_node=8, gat_heads=4, gat_dropout=0.6
    ).to(device)
    ...
    print("\n--- UGCN2 (k-NN Graph, GCN in bottleneck) ---")
    ugcn2_model = UGCN2(n_channels=num_mic, n_classes=1, num_node=8).to(device)
    ...
    print(f"UGCN2 Input shape: {x_ugcn2.shape}, Output shape: {output_ugcn2.shape}")

    print("\n--- UGAT2 (k-NN Graph, GAT in bottleneck) ---")
    ugat2_model = UGAT2(
        n_channels=num_mic, n_classes=1, num_node=8, gat_heads=4, gat_dropout=0.6
    ).to(device)
    ...
    # print(f"UGAT2 Input shape: {x_ugat2.shape}, Output shape: {output_ugat2.shape}")

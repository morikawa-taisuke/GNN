...
class SpeqGNN(nn.Module):
	...
	def forward(self, x_magnitude, complex_spec_input, original_length=None):
		...
		# 2. ボトルネックでのGNN処理
		_, channels_bottleneck, height_bottleneck, width_bottleneck = x4.size() # 形状の取得
		...
		edge_index = self.graph_builder.create_batch_graph(x_features_4d=x4)    # グラフ構築

		# GNN 層 (self.gnn) はフラットな [B*N, C] 形式を期待するため、グラフ構築とは別に、ここで変形処理を行う。
		x4_reshaped = x4.view(batch_size, channels_bottleneck, -1).permute(0, 2, 1).reshape(-1, channels_bottleneck)    # ノード用にリシェイプ

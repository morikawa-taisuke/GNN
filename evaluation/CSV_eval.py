                ...
                if len(row) > max(target_idx, estimation_idx):
                    ...
                    if target_file and estimation_filename_in_csv:
                        ...
                        if os.path.exists(target_file) and os.path.exists(estimation_file):
                            ...
                            print(f"⚠️ 警告: 行をスキップします。ファイルパスが存在しないか、ファイルが見つかりません: target='{target_file}', estimation='{estimation_file}'")
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_column", type=str, default="clean", help="ターゲット（クリーンな）音声ファイルへのパスを含む列の名前。(デフォルト: 'clean')")
    parser.add_argument("--estimation_column", type=str, default=estimation_column, help="評価対象のファイル名が含まれる列の名前。例: 'noise_only'")
    parser.add_argument("--estimation_dir", type=str, default=estimation_dir, help="評価対象の音声ファイルが格納されているディレクトリのパス。")
    parser.add_argument("--out_path", type=str, default=out_path, help="出力評価CSVファイルを保存するパス。例: 'C:/results/evaluation.csv'")

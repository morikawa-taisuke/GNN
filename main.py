...
def test(
    ...
    for _, (mix_data, mix_path) in tenumerate(
        dataset_loader
...
if __name__ == "__main__":
    ...
    for model_type in model_list:
        wave_type = (
            "noise_only"  # 入寮信号の種類 (noise_only, reverbe_only, noise_reverbe)
        )
        ...
        train(
            ...
            out_path=f"{const.PTH_DIR}/{model_type}/subset_DEMAND_1ch/{out_name}.pth",
            batchsize=1,
        )

        test(
            model=model,
            mix_dir=f"{const.MIX_DATA_DIR}/subset_DEMAND_hoth_1010dB_1ch/subset_DEMAND_hoth_1010dB_05sec_1ch/test/{wave_type}",
        ...
        evaluation(
            target_dir=f"{const.MIX_DATA_DIR}/subset_DEMAND_hoth_1010dB_1ch/subset_DEMAND_hoth_1010dB_05sec_1ch/test/clean",

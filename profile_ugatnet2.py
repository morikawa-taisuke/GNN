...
from models.GCN import UGATNet2 # models.urelnet から URelNet をインポートする代わりに、GCNからUGATNet2をインポート
import time
...
def profile_model(model, device="cpu", batch_size=1, num_mic=1, length=128000):
    ...
    with profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/profile_ugatnet2'), # ログの出力先を変更
        record_shapes=True,
        profile_memory=True,
        with_stack=True
    ) as prof:
        ...
        with record_function("model_inference"):
            output = model(x)
    print(prof.key_averages().table(sort_by="self_cuda_time_total",))  # 出力サイズを確認
    # プロファイリング結果の表示 (手動)
    print("\n=== プロファイリング結果 (UGATNet2) ===") # モデル名を明記
    ...
    print("TensorBoardで確認するには、ターミナルで `tensorboard --logdir=./log/profile_ugatnet2` を実行してください。")
    print("Chrome Traceで確認するには、Chromeブラウザで chrome://tracing を開き、trace_ugatnet2.jsonをロードしてください。")
...
if __name__ == '__main__':
    ...
    # UGATNet2 の初期化パラメータを適切に設定してください
    model_to_profile = UGATNet2(

...
def train(model: nn.Module,
	...
	"""蟄ｦ鄙帝未謨ｰ
	Args:
		model (nn.Module): 蟄ｦ鄙偵＆縺帙ｋ繝｢繝・Ν
		train_csv (str): 蟄ｦ鄙堤畑CSV繝輔ぃ繧､繝ｫ縺ｮ繝代せ
		val_csv (str): 讀懆ｨｼ逕ｨCSV繝輔ぃ繧､繝ｫ縺ｮ繝代せ
		wave_type (str): 蜈･蜉帑ｿ｡蜿ｷ縺ｮ遞ｮ鬘・(noise_only, reverbe_only, noise_reverb)
		out_path (str): 蟄ｦ鄙貞ｾ後・繝｢繝・Ν縺ｮ菫晏ｭ伜・繝代せ
		loss_type (str): 謳榊､ｱ髢｢謨ｰ縺ｮ遞ｮ鬘・("stft_MSE", "L1", "MSE", "SISDR")
		batchsize (int): 繝舌ャ繝√し繧､繧ｺ
		checkpoint_path (str): 繝√ぉ繝・け繝昴う繝ｳ繝医・繝代せ・・one縺ｮ蝣ｴ蜷医・譁ｰ隕丞ｭｦ鄙抵ｼ・
		train_count (int): 蟄ｦ鄙偵お繝昴ャ繧ｯ謨ｰ
		earlystopping_threshold (int): Early Stopping縺ｮ髢ｾ蛟､
		accumulation_steps (int): 蜍ｾ驟阪ｒ闢・ｩ阪☆繧九せ繝・ャ繝玲焚
	"""
	"""GPU縺ｮ險ｭ螳・""
	device = confirmation_GPU.get_device()
	""" 縺昴・莉悶・險ｭ螳・"""
	...
	for epoch in range(start_epoch, train_count + 1):  # 蟄ｦ鄙貞屓謨ｰ
		...
		optimizer.zero_grad()  # 蜍ｾ驟阪ｒ繧ｨ繝昴ャ繧ｯ縺ｮ髢句ｧ区凾縺ｫ繝ｪ繧ｻ繝・ヨ
		...
		for i, (mix_data, target_data) in tenumerate(train_loader):
			...
			# 蜍ｾ驟崎塘遨阪・縺溘ａ縺ｫ謳榊､ｱ繧偵せ繧ｱ繝ｼ繝ｫ
			model_loss = model_loss / accumulation_steps

			""" 隱､蟾ｮ騾・ｼ晄眺 """
			model_loss.backward()

			# 繝ｭ繧ｰ險倬鹸逕ｨ縺ｫ繧ｹ繧ｱ繝ｼ繝ｫ繧呈綾縺励◆謳榊､ｱ繧貞刈邂・
...
def test(model: nn.Module, test_csv: str, wave_type: str, out_dir: str, model_path: str, prm: int = const.SR, out_name: str = "output"):
	...
	for mix_data, mix_name in tqdm(dataset_loader):
		...
		separate = model(mix_data, export_name=mix_name[0], out_dir=out_name)  # 繝｢繝・Ν縺ｮ驕ｩ逕ｨ
		continue
...
if __name__ == "__main__":
	...
	model_list = [
        "UGCN", "UGAT",
	]  # 繝｢繝・Ν縺ｮ遞ｮ鬘・ "UGCN", "UGAT", "ConvTasNet", "UNet"
	wave_types = [
		"clean",
		# "reverb_only",
		# "noise_reverb",
	]  # 蜈･蜉帑ｿ｡蜿ｷ縺ｮ遞ｮ鬘・(noise_only, reverb_only, noise_reverb)    # UGAT_all_random_reverb_only
	node_selection_list = [
		NodeSelectionType.TEMPORAL,
		NodeSelectionType.ALL
	]  # 繝弱・繝蛾∈謚槭・譁ｹ豕・(ALL, TEMPORAL)
	edge_selection_list = [
		EdgeSelectionType.RANDOM,
		EdgeSelectionType.KNN
	]  # 繧ｨ繝・ず驕ｸ謚槭・譁ｹ豕・(RANDOM, KNN)
		...
		for edge_selection in edge_selection_list:
			...
			for model_type in model_list:
				...
				for wave_type in wave_types:
					...
					out_name = f"{model_type}_{wave_type}_{num_node}node_{node_selection.value}_{edge_selection.value}"  # 蜃ｺ蜉帛錐
					out_dir = f"{model_type}_{node_selection.value}_{edge_selection.value}_{wave_type}"  # 蜃ｺ蜉帛錐
